...
/**
 * Handles the client side of the CardBox services.
 */
public class CardBoxDirector extends BasicDirector
    implements GameReadyObserver
{
    public CardBoxDirector (CardBoxContext ctx)
    {
        super(ctx);
        _ctx = ctx;
    ...
    public void enteredLobby (LobbyConfig config)
    {
        ...
        if (_gameLoader == null) {
            _gameLoader = CardBoxUtil.createClassLoader(_cacheDir, gameId, gamedef);
    ...
    public void clientDidLogon (Client client)
    {
        ...
        _cacheDir = new File(CardBoxClient.localDataDir("cache"));
        if (!_cacheDir.exists()) {
            if (!_cacheDir.mkdirs()) {
        ...
        CardBoxService.ResultListener rl = new CardBoxService.ResultListener() {

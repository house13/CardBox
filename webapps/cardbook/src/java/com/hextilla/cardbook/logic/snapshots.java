    ...
    implements Logic
{
    ...
    public void invoke (Application app, InvocationContext ctx)
        throws Exception
    {
        final CardbookApp gtapp = (CardbookApp)app;
        ...
        if (POPULAR.equals(category)) {
            games = getGames(POPULAR, new RefreshFunc() {
                public List<GameRecord> refresh () throws Exception {
                    return gtapp.getCardBoxRepository().loadPopularGames(SNAPSHOT_COUNT);
                }
            });
            ...
            games = getGames(UPDATED, new RefreshFunc() {
                public List<GameRecord> refresh () throws Exception {
                    return gtapp.getCardBoxRepository().loadRecentlyUpdated(SNAPSHOT_COUNT);
                }
            });
            ...
            games = getGames(ADDED, new RefreshFunc() {
                public List<GameRecord> refresh () throws Exception {
                    return gtapp.getCardBoxRepository().loadRecentlyAdded(SNAPSHOT_COUNT);
                }
            });
        }

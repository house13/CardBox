...
package com.hextilla.cardbook.tools;
...
public class UpdateSnapshots
{
    public static void main (String[] args)
    {
        ...
        try {
            ...
            pctx.init(CardBoxRepository.GAME_DB_IDENT,
                      new StaticConnectionProvider(CardBoxConfig.getJDBCConfig()), null);
            CardBoxRepository cardrepo = new CardBoxRepository(pctx);
            pctx.initializeRepositories(true);

            String path = docroot + File.separator + "recently_added_incl.html";
            writeGameList(cardrepo.loadRecentlyAdded(9), new File(path));

            path = docroot + File.separator + "recently_updated_incl.html";
            writeGameList(cardrepo.loadRecentlyUpdated(9), new File(path));

            path = docroot + File.separator + "most_popular_incl.html";
            writeGameList(cardrepo.loadPopularGames(9), new File(path));

            path = docroot + File.separator + "all_games_incl.html";
            writeExtGameList(cardrepo.loadGames(), new File(path));

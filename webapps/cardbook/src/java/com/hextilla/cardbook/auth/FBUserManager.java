...
public class FBUserManager
{
     ...
     */
    public FBUser loadUser (HttpServletRequest req)
        throws PersistenceException
    {
     ...
     */
    public FBUser loadUser (String authcode)
        throws PersistenceException
    {
        FBUser user = (authcode == null) ? null : _repository.loadUserBySession(authcode);
        if (USERMGR_DEBUG) {
     ...
     */
    public FBUser requireUser (HttpServletRequest req)
        throws PersistenceException, RedirectException
    {
        FBUser user = loadUser(req);
     ...
     * @param expires Number of seconds until the Facebook auth token expires.
     ...
     */
    public FBUser login (String fbId, String token, int expires, HttpServletRequest req, HttpServletResponse rsp)
        throws PersistenceException, AuthenticationFailedException
    {
        // load up the requested user
        FBUser user = _repository.loadUser(fbId);
        if (user == null) {
    ...
    public Tuple<FBUser,String> login (
        ...
        throws PersistenceException, AuthenticationFailedException
    {
        ...
        return new Tuple<FBUser,String>(user, authcode);
    }
    */
     ...
     *
     * @param expires the number of seconds in which to expire the session cookie, 0 means expire at
     * the end of the browser session.
     */
    public void effectLogin (
        FBUser user, String token, int expires, HttpServletRequest req, HttpServletResponse rsp)
        throws PersistenceException
    {
        String authcode = _repository.registerSession(user, token, expires);
        ...
        acookie.setPath("/");
        acookie.setMaxAge((expires > 0) ? expires : -1);
        if (USERMGR_DEBUG) {
    ...
    public boolean refreshSession (String sessionKey, int expireSecs)
        throws PersistenceException
    {
        return _repository.refreshSession(sessionKey, expireSecs);
    }
     ...
     */
    protected FBUserRepository createRepository (ConnectionProvider conprov)
        throws PersistenceException
    {
        return new FBUserRepository(conprov);
    }
